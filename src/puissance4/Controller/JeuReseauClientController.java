 ...
 */
public class JeuReseauClientController implements Initializable {
    ...
    public void set(Scene scene, String pseudo1, String pseudo2, Color couleur1, Color couleur2, BufferedReader in, PrintWriter pred, Socket socket, String ip) {
        this.scene = scene;
    ...
    public void communicationJoueur() {
        try {
            ...
            System.out.println("Rep " + repJ1);

            String[] infoJ1 = repJ1.split("-");
            FlowPane source;
            int x = Integer.valueOf(infoJ1[0]);
            int y = Integer.valueOf(infoJ1[1]);
            ...
            if (firstCase != null) {
                ...
                Platform.runLater(new Runnable() {
                    ...
                    public void run() {
                        ...
                        System.out.println(casePane.getChildren());
                        Button bouton = (Button) casePane.getChildren().get(0);
                        bouton.setVisible(true);
                        bouton.setStyle("-fx-background-color: " + couleur + "; -fx-background-radius : 700;");
    ...
    public void ecranVictoire(Joueur j) {
        try {
            ...
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/puissance4/View/EcranVictoireClient.fxml"));

            scene.setRoot(loader.load());

            EcranVictoireClientController evcc = loader.getController();
            evcc.set(scene, j, j1, j2, socket, ip);
        } catch (IOException ex) {
    ...
    public void ecranEgalite() {
        try {
            ...
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/puissance4/View/EcranEgaliteClient.fxml"));

            scene.setRoot(loader.load());

            EcranEgaliteClientController eecc = loader.getController();
            eecc.set(scene, j1, j2, socket, ip);
        } catch (IOException ex) {
            Logger.getLogger(AccueilController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
